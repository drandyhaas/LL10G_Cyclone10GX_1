...
# ACDS 23.4 79 win32 2024.02.13.11:57:36
# ----------------------------------------
...
# ----------------------------------------
# If myfifo is one of several IP cores in your
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "myfifo.myfifo"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [myfifo::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [myfifo::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [myfifo::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [myfifo::get_memory_files "$QSYS_SIMDIR"]]
  ...
  set normalized_src [myfifo::normalize_path "$file"]
  set normalized_dest [myfifo::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [myfifo::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [myfifo::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [myfifo::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [myfifo::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
